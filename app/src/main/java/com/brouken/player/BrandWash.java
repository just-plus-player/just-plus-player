...
public class BrandWash extends View {
    ...
    public BrandWash(final Context context, final AttributeSet attrs) {
        super(context, attrs);
        // The theme's own tones, not a fixed plum — the plum was the coral world's ground and reads as
        // somebody else's colour under Nord or Forest. But the DARK variant of them, whatever the window
        // is doing, because that is this class's whole contract: read from the live Settings theme they
        // followed the appearance instead, and in light mode the wash came out #F9E4E2 -> #FFF8F6 under
        // white lettering — 1.22:1 and 1.05:1, an invisible block where the docstring promises 8:1. The
        // dark pair gives 6.3:1 at its palest (Everforest's container) and 18.9:1 at its darkest.
        final Context dark = new android.view.ContextThemeWrapper(
                context, Prefs.accentOverlay(context, false));
        near = MaterialColors.getColor(dark, R.attr.accentContainer,
                ContextCompat.getColor(context, R.color.brand_container));
        far = MaterialColors.getColor(dark, R.attr.accentGround,

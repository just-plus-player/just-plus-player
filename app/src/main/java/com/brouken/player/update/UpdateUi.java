...
public final class UpdateUi {
    ...
    public static void showAvailableDialog(final Activity activity, final Context ctx,
        ...
        // appcompat's R and not Material's: 1.14 stopped re-declaring colorPrimary in its own
        // attrs, and the two resolve to the same attribute either way.
        final int accent = MaterialColors.getColor(ctx, androidx.appcompat.R.attr.colorPrimary, ink);
        ...
        if (!changelog.isEmpty()) {
            ...
            // setMovementMethod makes the text focusable, and a ScrollView offers itself to a remote only
            // while nothing inside it is: Up from the actions then searched for the text, whose top sits
            // below the fold on a television, and found nothing. Links still open to a touch without it.
        ...
        // And back down to the same action: left to the focus search, leaving the notes landed on Ignore,
        // where a press out of habit would hide this version for good.
        forward.setId(View.generateViewId());
        scroll.setNextFocusDownId(forward.getId());
        dialog.show();

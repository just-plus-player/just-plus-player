...
class Prefs {
    ...
    /**
     * Whether a saved media server that will not answer is looked for on the network by the identity
     * it announced, rather than only at the address it was added at.
     *
     * <p>On by default, and the default is the whole argument: most media servers take whatever port
     * is free when they start, so a restart of the box the server runs on leaves every saved folder
     * pointing at nothing. Off, that reads as "the server is broken" and the only cure is to know to
     * add it again by hand. The search costs nothing on the path that works - {@code DlnaFiles.moved}
     * is reached only from the catch of a listing that has already failed - so what it spends is four
     * seconds before an error the viewer was going to get anyway.
     */
    public static boolean dlnaRediscover(final Context context) {
        return PreferenceManager.getDefaultSharedPreferences(context)
                .getBoolean("dlnaRediscover", true);
    }
